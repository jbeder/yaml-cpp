...
void Parser::HandleYamlDirective(const Token& token) {
  if (token.params.size() != 1) {
    raise<ParserException>(token.mark, ErrorMsg::YAML_DIRECTIVE_ARGS);
  }

  if (!m_pDirectives->version.isDefault) {
    raise<ParserException>(token.mark, ErrorMsg::REPEATED_YAML_DIRECTIVE);
  }
  ...
  if (m_pDirectives->version.major > 1) {
    raise<ParserException>(token.mark, ErrorMsg::YAML_MAJOR_VERSION);
  }
...
void Parser::HandleTagDirective(const Token& token) {
  if (token.params.size() != 2)
    raise<ParserException>(token.mark, ErrorMsg::TAG_DIRECTIVE_ARGS);

  const std::string& handle = token.params[0];
  const std::string& prefix = token.params[1];
  if (m_pDirectives->tags.find(handle) != m_pDirectives->tags.end()) {
    raise<ParserException>(token.mark, ErrorMsg::REPEATED_TAG_DIRECTIVE);
  }
