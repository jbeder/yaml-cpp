...
void Scanner::ScanNextToken() {
  ...
  raise<ParserException>(INPUT.mark(), ErrorMsg::UNKNOWN_TOKEN);
}
...
Token::TYPE Scanner::GetStartTokenFor(IndentMarker::INDENT_TYPE type) {
  ...
  assert(false);
  raise<std::runtime_error>("yaml-cpp: internal error, invalid indent type");
}
...
void Scanner::ThrowParserException(const std::string& msg) const {
  ...
  raise<ParserException>(mark, msg);
}
}  // namespace YAML
