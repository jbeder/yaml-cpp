...
void SingleDocParser::HandleDocument(EventHandler& eventHandler) {
  ...
  if (!m_scanner.empty() && m_scanner.peek().type != Token::DOC_END
    ...
    raise<ParserException>(m_scanner.mark(), ErrorMsg::UNEXPECTED_TOKEN_AFTER_DOC);
...
void SingleDocParser::HandleBlockSequence(EventHandler& eventHandler) {
  ...
  while (true) {
    if (m_scanner.empty())
      raise<ParserException>(m_scanner.mark(), ErrorMsg::END_OF_SEQ);

    Token token = m_scanner.peek();
    if (token.type != Token::BLOCK_ENTRY && token.type != Token::BLOCK_SEQ_END)
      raise<ParserException>(token.mark, ErrorMsg::END_OF_SEQ);
...
void SingleDocParser::HandleFlowSequence(EventHandler& eventHandler) {
  ...
  while (true) {
    if (m_scanner.empty())
      raise<ParserException>(m_scanner.mark(), ErrorMsg::END_OF_SEQ_FLOW);
    ...
    if (token.type == Token::FLOW_ENTRY)
      ...
      raise<ParserException>(token.mark, ErrorMsg::END_OF_SEQ_FLOW);
  }
...
void SingleDocParser::HandleBlockMap(EventHandler& eventHandler) {
  ...
  while (true) {
    if (m_scanner.empty())
      raise<ParserException>(m_scanner.mark(), ErrorMsg::END_OF_MAP);

    Token token = m_scanner.peek();
    if (token.type != Token::KEY && token.type != Token::VALUE &&
        token.type != Token::BLOCK_MAP_END)
      raise<ParserException>(token.mark, ErrorMsg::END_OF_MAP);
...
void SingleDocParser::HandleFlowMap(EventHandler& eventHandler) {
  ...
  while (true) {
    if (m_scanner.empty())
      raise<ParserException>(m_scanner.mark(), ErrorMsg::END_OF_MAP_FLOW);
    ...
    if (nextToken.type == Token::FLOW_ENTRY)
      ...
      raise<ParserException>(nextToken.mark, ErrorMsg::END_OF_MAP_FLOW);
  }
...
void SingleDocParser::ParseTag(std::string& tag) {
  Token& token = m_scanner.peek();
  if (!tag.empty())
    raise<ParserException>(token.mark, ErrorMsg::MULTIPLE_TAGS);
...
void SingleDocParser::ParseAnchor(anchor_t& anchor, std::string& anchor_name) {
  Token& token = m_scanner.peek();
  if (anchor)
    raise<ParserException>(token.mark, ErrorMsg::MULTIPLE_ANCHORS);
...
anchor_t SingleDocParser::LookupAnchor(const Mark& mark,
  ...
  if (it == m_anchors.end()) {
    ...
    raise<ParserException>(mark, ss.str());
  }
