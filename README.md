...
* The 3rd step above can do like this:
  ...
  ```bash
  cmake -DCMAKE_TOOLCHAIN_FILE=${NDKROOT}/build/cmake/android.toolchain.cmake -DANDROID_STL=c++_shared -DANDROID_PLATFORM=android-26 -DANDROID_ABI=x86_64 -DYAML_BUILD_ANDROID_LIBS=ON ..
  ```
  ...
  - To do cross-compiling, `ndk-bundle` OR `android-sdk` is needed. Also `NDKROOT` should be exported in your `bash` config file, just like:
    ...
    ```bash
    export NDKROOT=/path/to/Android/sdk/ndk-bundle
    ```

  - The argument `ANDROID_ABI` can be set to `arm64-v8a`, `armeabi-v7a`, `armeabi`, `x86`, `x86_64`， etc. If NOT set, the default value is `armeabi-v7a`.

  - The argument `ANDROID_PLATFORM` can be set to `android-xx` OR `latest`, `xx` can be replaced to the android api number, the default value will be the minimum supported version of your `ndk-bundle`, while the value `latest` will using the maximum supported version.

  - The argument  `YAML_BUILD_ANDROID_LIBS=ON` will default set `YAML_BUILD_SHARED_LIBS=ON`, and copy the `libc++_shared.so` to `build` directory from `ndk-bundle`, which is needed by `libyaml-cpp.so`.
