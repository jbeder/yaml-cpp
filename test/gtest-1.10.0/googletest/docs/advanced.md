...
arguments passed to mock objects. A gMock _matcher_ is basically a predicate
...
error like `"error: void value not ignored as it ought to be"` or `"cannot initialize return object of type 'bool' with an rvalue of type 'void'"` or
...
can be _any valid statement_ (including _compound statement_) and doesn't have
to be an expression.
...
for generating test parameters. They return what we call (surprise!) _parameter
...
Please note that `INSTANTIATE_TEST_SUITE_P` will instantiate _all_ tests in the
given test suite, whether their definitions come before or _after_ the
