...
def gen_tests():
    for events in gen_events():
        name = (
            "test"
            + hashlib.sha1("".join(yaml.dump(event) for event in events)).hexdigest()[
                :20
            ]
        )
...
def create_emitter_tests(out):
    ...
    with Scope(out, "namespace YAML", 0) as _:
        with Scope(out, "namespace", 0) as _:
            ...
            for test in tests:
                with Scope(
                    out, "TEST_F(%s, %s)" % ("GenEmitterTest", test["name"]), 2
                ) as _:
