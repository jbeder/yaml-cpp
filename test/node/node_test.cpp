...
TEST(NodeTest, CallExceptionHandler) {
  ...
  YAML::set_handle_exception([](const char* what) {
  ...
  YAML::set_handle_exception_local([](const char* what) {
    throw std::domain_error("error");
  });
  EXPECT_THROW(node["bar"].as<std::string>(), std::domain_error);

  // Check if exception was thrown
  YAML::set_handle_exception(nullptr);
  YAML::set_handle_exception_local(nullptr);
  EXPECT_THROW(node["bar"].as<std::string>(), InvalidNode);
}
